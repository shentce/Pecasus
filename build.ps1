...
BuildSolution()
{
	...
	if [[ "$buildConfiguration" == "" ]]; then
		local buildConfiguration="Nuget_Release"
	fi
	...
	"${msBuildPath}" "${slnFile}" -m -target:Restore -property:Configuration="${buildConfiguration}"

	if [[ "$ENABLE_CLEAN" == "true" ]]; then
		echo "Cleaning '${slnFile}' projects..."
		"${msBuildPath}" "${slnFile}" -m -target:Clean -property:Configuration="${buildConfiguration}"
	fi

	echo "Building '${slnFile}' projects..."
	"${msBuildPath}" "${slnFile}" /nodeReuse:false -m -target:Build -property:Configuration="${buildConfiguration}"
...
BuildDriverIntestTstc()
{
	local slnFile="CommonCoreIvi.IVI.Interface.sln"
	BuildSolution "${slnFile}" "" "Release"

	local slnFile="IVI\Drivers\InTestTstc\InTestTstc.sln"
	BuildSolution "${slnFile}" "" "Release"
}
...
if [ $# -eq 0 ]; then
	...
	echo "        --driver-intesttstc      = Build InTestTsTc driver nuget packages. Unit tests will be run automatically."
	echo "        --test-driver-intesttstc      = Build InTestTsTc driver nuget packages. Unit tests will be run automatically."
	echo ""
	exit 1
else
	POSITIONAL=()
	...
	echo ""
	{
		...
		if [[ "$BUILD_DRIVER_INTESTTSTC" == "true" ]]; then
			BuildDriverIntestTstc
		fi
